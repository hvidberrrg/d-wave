...
def generate_maximum_cut_bqm(G: nx.Graph) -> dimod.BinaryQuadraticModel:
...
def maximum_cut(G: nx.Graph) -> tuple[set[int], set[int], set[tuple[int, int]], set[tuple[int, int]]]:
    """Solves the maximum cut problem for the given graph, G. That is, the vertices
    of G is separated into two complementary subsets, s and s_c, such that the number
    ...
    Returns:
            ...
            maximum cut for the graph G, along with two complementary sets of edges, comprising
            the sets of cut and uncut edges respectively.
    """
    bqm = generate_maximum_cut_bqm(G)
    sample_label = "Maximum cut (" + str(G.number_of_nodes()) + " nodes, " + str(G.number_of_edges()) + " edges)"
    maximum_cut_info = sampler.sample_dwave(bqm, sample_label)
    assert (G.number_of_nodes() == len(maximum_cut_info)), "Something went wrong... the maximum cut info doesn't match the input graph."
